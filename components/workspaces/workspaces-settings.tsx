...
/**
 * The workspace list, edit rows and create form on their own, so the same UI can
 * be the body of ManageWorkspacesDialog and the Workspaces section of the
 * Settings dialog. `listClassName` is how each host bounds the scrolling list.
 */
export function WorkspacesSettings({
  ...
  return (
    <>
      <div
        className={cn(
          "flex flex-col gap-1 overflow-y-auto py-1",
          listClassName
        )}
      >
        ...
        >
          ...
          {!bookmarksAvailable && !error && (
            <p className="text-xs text-muted-foreground">
              Bookmarks aren&apos;t available in this context, so new workspaces
...
function WorkspaceEditRow({
  ...
  return (
    <form
      ...
      className="flex items-center gap-2 rounded-lg bg-accent/40 p-1.5"
    >
