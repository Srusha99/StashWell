...
// Hoisted to module scope: neither shader depends on props, so there's no
// reason to allocate a new copy of either string on every render.
const VERT = /* glsl */ `
...
/**
 * A colorful animated OGL orb, optionally reactive to the microphone.
 *
 * `enableVoiceControl` defaults true to match the source this was adapted from,
 * but every call site in this app passes it explicitly - see the launcher button
 * in dashboard-view.tsx for why it's off there. `getUserMedia` throws a real
 * browser permission prompt, and this component has no fallback UI for a denial;
 * a caller that turns voice on must be deliberate about it.
 */
export const VoicePoweredOrb: FC<VoicePoweredOrbProps> = ({
  ...
  // Uint8Array<ArrayBuffer>, not the bare Uint8Array (which now defaults its
  // buffer parameter to ArrayBufferLike): AnalyserNode.getByteFrequencyData
  // wants the narrower type, and this repo's DOM lib target enforces it.
  const dataArrayRef = useRef<Uint8Array<ArrayBuffer> | null>(null)
  const mediaStreamRef = useRef<MediaStream | null>(null)

  // Voice analysis: RMS over the frequency bins, boosted by sensitivity.
  const analyzeAudio = () => {
  ...
  const stopMicrophone = () => {
    try {
      ...
      // Best-effort cleanup - a half-torn-down audio graph isn't worth surfacing.
    }
  }
  ...
  useEffect(() => {
    ...
    // OGL's own branded context type, not the bare DOM union - ogl's Renderer.gl
    // carries extra fields (canvas, renderer) that WebGLRenderingContext |
    // WebGL2RenderingContext doesn't, which is what every "not assignable" error
    // below was actually about.
    ...
    try {
      ...
      const update = (t: number) => {
        ...
        if (enableVoiceControl && isMicrophoneInitialized) {
          ...
          // Idle rotation so the orb still reads as "alive" with voice off,
      ...
      return () => {
        ...
        if (container && glContext?.canvas) {
          try {
            ...
            // Container may already be gone during unmount ordering.
          }
        }
